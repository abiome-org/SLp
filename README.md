...
- [MODELS.md](MODELS.md): the SL model battery: 80+ variants of published SL predictors, re-trained on
  SLB train where possible and scored on dev. Generated by `slpbench battery` from
  `models/battery.yaml`; re-run every adapter with `scripts/models/run_battery.sh`.
