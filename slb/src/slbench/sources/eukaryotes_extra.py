"""Additional non-human eukaryote genetic-interaction sources.

Each public function named after a source key returns the MEASUREMENT schema via `finalize`.
`*_checks()` functions compute the reproducibility evidence that `slbench audit` reports.

Species codes and canonical IDs:
  scer  SGD systematic ORF          spom  PomBase systematic ID
  dmel  FlyBase FBgn                cele  WormBase WBGene ID (slbench.ids_extra.cele)
  mmus  MGI symbol (slbench.ids_extra.mmus)

Sources and include/exclude decisions:
...
def frost2012_checks() -> list[dict]:
    ...
    from slbench.sources.yeast import ryan2012
...
def fitness_only_auroc(df: pl.DataFrame, single: pl.DataFrame) -> dict:
    """AUROC of -(f_a + f_b) for the labels (the audit's 'fitness-only' diagnostic)."""
...
def horn2011_checks() -> list[dict]:
    ...
    from slbench.sources.dmel import fischer2015, heigwer2023
...
def mmus_single() -> pl.DataFrame:
    ...
    from slbench import ids, ids_extra
    from slbench.fitness import depmap_long
...
def _costanzo2016() -> pl.DataFrame:
    ...
    from slbench.sources.yeast import costanzo2016
    return costanzo2016()
