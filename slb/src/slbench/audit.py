"""`slbench audit`: re-run every label-reproducibility check and write results/reports/replication.md.
...
from slbench import replication as R
from slbench.build import EXCLUDED_SOURCES, INTERIM, _context_table

MIN_AUROC = 0.65

# Per-source checks implemented next to their parsers.
NEW_CHECKS = [
...
def _new_check(name: str) -> list[dict]:
    ...
    mod, fn = name.split(".")
    rows = getattr(importlib.import_module(f"slbench.sources.{mod}"), fn)()
...
def write(res: dict) -> None:
    ...
    text = f"""# Label reproducibility

Generated by `slbench audit`. Do not edit by hand.
...
the rationale is in each function's docstring. "X labels scored by Y" = AUROC of
