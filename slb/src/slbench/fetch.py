"""Download raw sources into data/raw/<source>/ and record sha256 in data/raw/MANIFEST.tsv.
    ...
    uv run python -m slbench.fetch                 # everything
    uv run python -m slbench.fetch slkb biomart    # selected sources

Large figshare files download far faster with `python -m slbench.pget URL DEST`.
"""
...
def fetch(source: str, force: bool = False) -> None:
    ...
    for name, url in SOURCES[source]:
        ...
        subprocess.run(
            ["curl", "-fL", "--retry", "5", "--retry-delay", "5", "-sS", "-A", "slbench/0.1", "-o", str(tmp), url],
            check=True,
        )
