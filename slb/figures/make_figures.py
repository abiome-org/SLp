...
RESULTS = ROOT / "results/models/slb"
...
def figure_2() -> None:
    ...
    fig.text(.05, .89, "SLB held-out test  ·  Fitness-balanced AUROC  ·  Measured negatives",
             fontsize=11.5, color=MUTED)
...
def figure_3() -> None:
    ...
    fig.text(.07, .94, "The signal is uneven across species", fontsize=20,
             fontweight="bold", color=INK)
    fig.text(.07, .875, "SLB test  ·  Fitness-balanced AUROC  ·  Both gene families unseen",
...
def figure_4() -> None:
    ...
    path = ROOT / "reference/ancestry_audit.json"
    if not path.exists():
        return
    ...
    fig.text(.04, .885, "Frozen SLB test predictions  ·  Fitness-balanced AUROC within cell line and screen",
...
def _ancestry_benchmark() -> dict:
    return json.loads((ROOT / "reference/ancestry_benchmark.json").read_text())
...
def figure_5() -> None:
    ...
    if not (ROOT / "reference/ancestry_benchmark.json").exists():
        return
