...
#   SLB_BENCH=data/slb SLB_SPLIT=dev bash scripts/models/run_battery.sh [model ...]
...
mkdir -p "$(dirname "$log")"
models=${*:-$(uv run python -c "import yaml; r=yaml.safe_load(open('battery.yaml'))['models']; print(' '.join([*dict.fromkeys(e.get('adapter', m) for m, e in r.items() if e['family'] not in ('baseline', 'ensemble')), 'dev_rank_ensemble']))")}
failed=0
for m in $models; do
...
for b in random fitness fitness_lgbm paralog_identity codependency lgbm; do
  uv run slbench baseline $b --split "$SLB_SPLIT" --out results/$(basename "$SLB_BENCH")/${b}_${SLB_SPLIT}.parquet >> "$log" 2>&1 || {
    echo "$b baseline: FAILED (see $log)" | tee -a "$log"; failed=1;
  }
done
uv run slbench battery --split "$SLB_SPLIT" || failed=1
exit "$failed"
