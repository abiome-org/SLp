...
Writes data/interim/bundle/<species>/:
  ...
  fitness.parquet  gene, effect, source   reference single-loss effect (negative = sicker): slbench.fitness
                   for the original species (human, scer, spom, spne), else the first `<species>_single()` found in slbench.sources.*
...
Gene IDs are canonical SLB IDs via slbench.ids_extra.resolver(species) (ids.py for the base species).
...
class Res:
    ...
    def __init__(self, sp: str):
        ...
        self.extra = {}
        if sp == "spne":
            from slbench.sources.bacteria_extra import spne_resolver
...
def fitness(sp: str, res: Res) -> pl.DataFrame:
    from slbench import fitness as F

    ge = F.gene_effects()
    d = ge.filter(pl.col("species") == sp)
    if d.height:
        return d.select("gene", pl.col("effect").cast(pl.Float64), pl.lit(f"slbench.fitness.gene_effects").alias("source"))
    for mod in ("slbench.fitness", "slbench.sources.eukaryotes_extra", "slbench.sources.bacteria_extra",
                "slbench.sources.eukaryotes", "slbench.sources.bacteria"):
        try:
