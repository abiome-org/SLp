...
PATH = Path(__file__).parents[1] / "modules/slp-1-1-sl-readout-v2/readout.py"
...
def test_feature_loader_rejects_reordered_world_ids(tmp_path):
    ...
    np.savez(roster / "pairs.npz", pair_ids=ids, gene_indices=np.array([[0, 1], [2, 3]]))
    np.savez(roster / "genes.npz", gene_ids=np.array(["a", "b", "c", "d"]),
             action_features=np.ones((4, 642), np.float32))
    ...
    with pytest.raises(ValueError, match="not exactly aligned"):
        M.load_features(tmp_path / "base.npy", tmp_path / "world.npz", roster,
                        roster / "genes.npz", roster / "pairs.npz")
...
def test_static_pair_features_are_symmetric_and_exact_width(tmp_path):
    descriptors = np.arange(3 * 642, dtype=np.float32).reshape(3, 642) / 100
    np.savez(tmp_path / "genes.npz", gene_ids=np.array(["a", "b", "c"]),
             action_features=descriptors)
    np.savez(tmp_path / "pairs.npz", pair_ids=np.array([["a", "b"], ["b", "a"]]),
             gene_indices=np.array([[0, 1], [1, 0]]))
    static = M.StaticPairFeatures(tmp_path / "genes.npz", tmp_path / "pairs.npz")
    values = static[np.array([0, 1])]
    assert static.shape == (2, 1926)
    assert values.shape == (2, 1926)
    assert np.array_equal(values[0], values[1])
    assert np.array_equal(values[0, :642], descriptors[0] + descriptors[1])
    assert np.array_equal(values[0, 642:1284], np.abs(descriptors[0] - descriptors[1]))
    assert np.array_equal(values[0, 1284:], descriptors[0] * descriptors[1])


def test_fit_source_cannot_resolve_test_label_files():
    import inspect
    source = inspect.getsource(M.fit)
    assert "test_labels_seed" not in source
    assert "train_labels_seed" in source
