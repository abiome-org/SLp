...
Edge sources (built by scripts/orthology_extra/build_edges.py; details in notes/data/orthology.md):
...
def extra_edges_with_source(sources: tuple[str, ...] = ALL_SOURCES, base_pairs: bool = True,
    ...
    base_pairs: keep DIAMOND RBH orthologs between two base species (human/scer/spom/dmel). These add
      ...
      supported by >= 3 Alliance methods); they are conservative for leakage but change SLB-1.2 families.
