...
Written by the data-bacteria agent (2026-09). Nothing here is wired into build.py yet; the lead
integrates. Every source function returns the MEASUREMENT schema via `finalize`; every
`<species>_single()` returns [gene, effect] with negative = sicker.

Reproducibility checks for `slpbench audit` (each returns list[dict] with keys check/auroc/pos,
...
Species codes and canonical gene IDs:
  spne  S. pneumoniae D39V. Canonical IDs come from `slpbench.ids_extra.spne()` (lead decision
        2026-09-24): the ID already used in SLB-1.2, i.e. the dual CRISPRi-seq author name where
...
def spne_resolver() -> dict[str, str]:
    ...
    The canonical ID is whatever `slpbench.ids_extra.spne()` returns, which is the ID already used
...
def crisprtnseq2024() -> pl.DataFrame:
    ...
    Caveat, see notes/data/crisprtnseq2024_spneumo.md: these labels are highly reproducible within
...
def spne_single() -> pl.DataFrame:
    ...
    """
    from slpbench import fitness as _f
...
def koo2025(threshold: float = -1.5) -> pl.DataFrame:
    ...
    see notes/data/koo2025_bsub.md). Negative: |mean GI| below the median |GI| of the merged
...
# AUROC among them is 0.46-0.51 (notes/data/ecoli_arrays.md). They are parsed so the lead can keep
...
def cote2016() -> pl.DataFrame:
    ...
    not agree with them (notes/data/ecoli_arrays.md).
    """
    import openpyxl
...
# ============================================================================================
# Reproducibility checks (`slpbench audit` -> REPLICATION.md)
#
...
# "ci95" and "source". They recompute, from the raw files, the numbers written up in
# notes/data/{zik2025_spneumo,koo2025_bsub,crisprtnseq2024_spneumo,ecoli_arrays}.md.
# ============================================================================================
...
def spne_cross_checks() -> list[dict]:
    ...
    from slpbench import fitness
    from slpbench.sources.bacteria import dualcrispri2025
