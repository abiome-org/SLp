...
def scalar(pack,key):
    if key not in pack:return None
    value=np.asarray(pack[key])
    return value.item() if value.size==1 else None


def provenance(pack,path):
    source=str(scalar(pack,"source_id") or path.stem); context=scalar(pack,"context_id"); mode=scalar(pack,"mode"); duration=scalar(pack,"duration_hours"); dose=scalar(pack,"dose"); condition=None
    if mode is not None and duration is not None and np.isfinite(float(duration)):condition=f"mode={mode}|hours={float(duration):.9g}|dose={dose if dose is not None else 'not_applicable_or_unreported'}"
    return source,None if context is None else str(context),condition
...
def build(source,dimensions=32,fitness=False,additional=(),name=None,nested_prefix=0):
    ...
    for path in paths:
        z=np.load(path,allow_pickle=False); names=np.asarray([canon(x) for x in z["feature_name"]]); present=set(names); common=present if common is None else common&present; loaded.append((path,z,names,*provenance(z,path)))
    common=sorted(common); normalized=[]; rows=[]; audits=[]
    for sid,(path,z,names,source_id,context_id,experimental_condition) in enumerate(loaded):
        ...
        keep=role>=0; pair=np.full((keep.sum(),2),-1,"int16"); pair[:,0]=[ids[x] for x in a[keep]]; double=card[keep]==2; pair[double,1]=[ids[x] for x in b[keep][double]]; rows.append((pair,np.full(keep.sum(),sid,"int8"),card[keep].astype("int8"),role[keep],x[keep],fitness_target[keep],None if context_id is None else np.full(keep.sum(),context_id),None if experimental_condition is None else np.full(keep.sum(),experimental_condition)))
        audits.append({"source":source_id,"input_sha256":sha(path),"context_id":context_id,"experimental_condition_id":experimental_condition,"train_rows":int(fit.sum()),"validation_rows":int((role==1).sum()),"train_double_rows":int((fit&(card==2)).sum()),"validation_double_rows":int(((role==1)&(card==2)).sum())})
    ...
    arrays={"pairs":np.concatenate([x[0] for x in rows]),"source":np.concatenate([x[1] for x in rows]),"cardinality":np.concatenate([x[2] for x in rows]),"role":np.concatenate([x[3] for x in rows]),"target":target.astype("float32"),"sources":np.asarray([x[3] for x in loaded]),"state_dimensions":np.int16(dimensions),"state_blocks":blocks,"state_block_weights":np.full(len(blocks),1/len(blocks),"float32"),"fitness_index":np.int16(dimensions if fitness else -1)}
    if not fitness:arrays["target_semantics"]=np.asarray("perturbation_delta")
    if all(x[6] is not None for x in rows):arrays["context_id"]=np.concatenate([x[6] for x in rows])
    if all(x[7] is not None for x in rows):arrays["experimental_condition_id"]=np.concatenate([x[7] for x in rows])
    name=name or ("perturbseq_world_fitness" if fitness else "perturbseq_world"); np.savez_compressed(OUT/f"{name}.npz",**arrays)
    audit={"schema":"sl-predict-perturbseq-world-v1","sources":audits,"rows":len(target),"train_rows":int(train.sum()),"validation_rows":int((~train).sum()),"common_expression_genes":len(common),"state_dimensions":dimensions,"state_blocks":blocks.tolist(),"nested_residual_endpoint":bool(nested_prefix),"signed_fitness_target":"within-source robust z-score of log recovered cells by condition" if fitness else None,"target_semantics":"mixed" if fitness else "perturbation_delta","context_metadata_complete":"context_id" in arrays,"condition_metadata_complete":"experimental_condition_id" in arrays,"held_scenario3_genes":len(held),"benchmark_membership_used_for_exclusion_only":True,"sl_labels_used":False,"explained_variance":float(explained)}; (OUT/f"{name}.json").write_text(json.dumps(audit,indent=2)); print(json.dumps(audit,indent=2))
