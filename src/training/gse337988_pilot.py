...
def build(raw,source,output):
 ...
 for sid,moi in enumerate(MOIS):
  ...
  strict={str(n):int((kind==n).sum()) for n in range(max_card+1)}; audit.append({"source":out.stem,"rds_sha256":sha(rds),"h5_sha256":sha(h5),"cells":len(cells),"expression_genes":len(symbols),"common_genes":len(features),"strict_cells_by_cardinality":strict,"pair_cells":int((kind==2).sum()),"pair_conditions":int(sum("+" in x for x in unique)),"pair_conditions_ge4":int(sum("+" in x for x in reps)),"supported_single_and_pair_conditions_ge4":len(reps),"pseudobulks":len(keys),"set_train_cells":int((role==0).sum()),"set_validation_cells":int((role==1).sum())}); records.append((sid,h5,members,kind,role,full_control))
