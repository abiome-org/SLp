"""Role-separated protected truth from the pinned yeast knockout proteome.

This self-contained module is source-specific and architecture-neutral.  One
run converts exactly one held role.  Pretraining, the other held role, controls,
quarantine, and analytical-QC columns are tokenized only as required by the CSV
container and are never numerically converted or semantically validated.
"""
...
@dataclass(frozen=True)
class ObservationRoleContract:
    role: str
    records: int
    intervention_genes: int
    target_values: int
    missing_values: int
    trajectory_genes_sha256: str
    trajectory_gene_set_sha256: str
    action_sequence_sha256: str
    locator_sequence_sha256: str

    def __post_init__(self) -> None:
        if self.role not in {ROLE_PRETRAIN, ROLE_VALIDATION, ROLE_FINAL}:
            raise ProteomeObservationError("observation role is not governed")
        counts = (
            self.records,
            self.intervention_genes,
            self.target_values,
            self.missing_values,
        )
        if any(type(value) is not int or value <= 0 for value in counts):
            raise ProteomeObservationError("observation role counts must be positive integers")
        for value in (
            self.trajectory_genes_sha256,
            self.trajectory_gene_set_sha256,
            self.action_sequence_sha256,
            self.locator_sequence_sha256,
        ):
            if SHA256.fullmatch(value) is None:
                raise ProteomeObservationError("observation role digest is invalid")
...
PRODUCTION_PROTECTED_ROLES = {
    ROLE_VALIDATION: ObservationRoleContract(
        role=ROLE_VALIDATION,
        records=537,
        intervention_genes=529,
        target_values=967_019,
        missing_values=26_431,
        trajectory_genes_sha256="32ea7b84790202f0b9a87c95e31434c9aba3d8588a68a090964ce9edd2373558",
        trajectory_gene_set_sha256="932a2750d3ae3cff3bbbcc165f985bcefbef6f3fa3cf369512fa6d71b217fa76",
        action_sequence_sha256="4d5bb70c6c2fb22f1a938a7134f15601a9251cf771a3cdc92096e5aa8320cdc4",
        locator_sequence_sha256="f226453f029b8fa789acdc1e5ac264136e07584c57bebba8443264988efac2ed",
    ),
    ROLE_FINAL: ObservationRoleContract(
        role=ROLE_FINAL,
        records=275,
        intervention_genes=268,
        target_values=496_490,
        missing_values=12_260,
        trajectory_genes_sha256="e789e789b8f9089d8c6d3348f9922d84f3557882a4138bd6d9efa4e53aa03abc",
        trajectory_gene_set_sha256="f90d8eb53a39685855a07fd350108adb9bb8e87f771ea8391294d660e04f29f1",
        action_sequence_sha256="4c3084b89acd7a9a900767f640a35674d90cdb1dd6f838304cb64f1a87beb9eb",
        locator_sequence_sha256="849556280e0d4d70906d7325ae2bc229bd81f4c8085ddb4c3e3e10e926d0c6e1",
    ),
}
...
def decode_matrix(
    ...
    bounds: Bounds,
    *,
    selected_role: str = ROLE_PRETRAIN,
) -> tuple[int, int, np.ndarray, np.ndarray]:
    if selected_role not in {ROLE_PRETRAIN, ROLE_VALIDATION, ROLE_FINAL}:
        raise ProteomeObservationError("matrix decode role is not governed")
            ...
            for source_row, row in enumerate(reader, start=2):
                ...
                for selected_index, column in enumerate(selected_columns):
                    decoded = _decode_observed(
                        row[column],
                        f"{selected_role} cell row={source_row}, column={column + 1}",
                    )
...
def _pretrain_role_contract(expected: ExpectedCounts) -> ObservationRoleContract:
    return ObservationRoleContract(
        role=ROLE_PRETRAIN,
        records=expected.pretrain_records,
        intervention_genes=expected.pretrain_genes,
        target_values=expected.target_values,
        missing_values=expected.missing_values,
        trajectory_genes_sha256=expected.trajectory_genes_sha256,
        trajectory_gene_set_sha256=expected.trajectory_gene_set_sha256,
        action_sequence_sha256="b0e60090721d7fc09e2b91651901095e4d5ff86ffb83030d71ec0e8888ba0318",
        locator_sequence_sha256="a745e9f547f51af77397e252dbee03f31f2eb81d38aa08577783063c6f96f2e4",
    )


def _archive_id(role: str) -> str:
    if role not in {ROLE_PRETRAIN, ROLE_VALIDATION, ROLE_FINAL}:
        raise ProteomeObservationError("observation archive role is not governed")
    return f"slp-observations:mendeley-w8jtmnszd9-v2-{role}-v1"
...
def validate_observation_archive(
    ...
    *,
    ...
    role_contract: ObservationRoleContract | None = None,
    provenance: PreparationProvenance | None = None,
    expected_runtime: Mapping[str, str] | None = None,
) -> dict[str, Any]:
    governed_role = role_contract or _pretrain_role_contract(expected)
    ...
    if (
        ...
        or manifest["archiveId"] != _archive_id(governed_role.role)
        or manifest["version"] != "v1"
        or manifest["role"] != governed_role.role
    ...
    expected_counts = {
        "records": governed_role.records,
        "interventionGenes": governed_role.intervention_genes,
        "readouts": expected.protein_readouts,
        "assayedValues": governed_role.records * expected.protein_readouts,
        "observedValues": governed_role.target_values,
        "missingValues": governed_role.missing_values,
    }
    if (
        manifest["counts"] != expected_counts
        or governed_role.target_values + governed_role.missing_values
        != expected_counts["assayedValues"]
    ):
    ...
    if (
        ...
        or manifest["trajectoryGenes"]["records"] != governed_role.intervention_genes
    ):
    ...
    trajectory_genes = set(
        _validate_trajectory_genes(
            referenced_payloads["trajectoryGenes"], governed_role.intervention_genes
        )
    )
    if (
...
def build_protected_observations(
    raw_root: str | Path,
    ...
    *,
    role: str,
    role_contract: ObservationRoleContract,
    source_contract: SourceContract = PRODUCTION_SOURCE_CONTRACT,
    expected: ExpectedCounts = PRODUCTION_EXPECTED_COUNTS,
    bounds: Bounds = Bounds(),
    provenance: PreparationProvenance | None = None,
) -> dict[str, Any]:
    if role not in {ROLE_VALIDATION, ROLE_FINAL} or role_contract.role != role:
        raise ProteomeObservationError("protected preparation requires one exact held role")
    destination_path = Path(destination)
    if destination_path.exists():
        raise ProteomeObservationError("refusing to overwrite a protected observation preparation")
    ...
    partition_genes = {
        ...
        for partition_role, rows in partitions.items()
    }
    count_tuple = (
        len(partitions[ROLE_PRETRAIN]),
        len(partition_genes[ROLE_PRETRAIN]),
        len(controls),
        len(partition_genes[ROLE_VALIDATION]),
        len(partitions[ROLE_VALIDATION]),
        len(partition_genes[ROLE_FINAL]),
        len(partitions[ROLE_FINAL]),
        len(quarantine),
        len(qc),
    )
    expected_tuple = (
    ...
    if any(
        set(partition_genes[left]) & set(partition_genes[right])
        for left, right in (
            (ROLE_PRETRAIN, ROLE_VALIDATION),
            (ROLE_PRETRAIN, ROLE_FINAL),
            (ROLE_VALIDATION, ROLE_FINAL),
        )
    ):
    ...
    locator_sequence_sha256 = canonical_sha256(
        [
            {
                "actionId": str(sample.action_id),
                "matrixColumn": sample.matrix_column,
                "metadataRow": sample.metadata_row,
            }
    ...
    if (
        ...
        != role_contract.records * expected.protein_readouts
    ):
    ...
    try:
        ...
        plate_vocabulary = sorted({sample.plate for sample in selected})
        plate_index = {value: index for index, value in enumerate(plate_vocabulary)}
        with tempfile.TemporaryDirectory(prefix="slp-proteome-protected-memmap-") as temporary:
            memmap_path = Path(temporary) / "targets.f32"
            matrix = np.memmap(
                memmap_path,
                mode="w+",
                dtype="<f4",
                shape=(len(selected), len(relation_rows)),
            )
            try:
                matrix[:] = np.nan
                target_values, missing_values, _, _ = decode_matrix(
                ...
                matrix.flush()
                if (
                    target_values != role_contract.target_values
                    or missing_values != role_contract.missing_values
                ):
                    raise ProteomeObservationError(
                        "frozen protected observed/missing value count drift"
                    )
        ...
        if validated_manifest["counts"] != manifest["counts"]:
            raise ProteomeObservationError(
                "post-write protected archive validation changed the content contract"
            )
        unselected_rows = {
            partition_role: len(rows)
            for partition_role, rows in partitions.items()
            if partition_role != role
        }
        unselected_rows.update(
            {
        ...
        audit = {
            ...
            "role": role,
            "intendedUse": "protected-molecular-evaluation-truth",
            "source": source,
            "identity": identity,
            "partition": partition,
            "runtime": runtime,
            "accessBoundary": {
                ...
                "numericColumnsConverted": {role: len(selected)},
                "numericColumnsNotConverted": dict(sorted(unselected_rows.items())),
                "unselectedNumericTokensConvertedOrSemanticallyValidated": False,
                "fittingOrRewardInputsPresent": False,
                "fittingOrRewardOperationsPresent": False,
                "benchmarkLabelsPresent": False,
                "filenameSuffixesUsed": False,
                "controlsDecoded": False,
            },
            "counts": {
                ...
                "selectedInterventionGenes": len(selected_genes),
                "selectedObservedValues": target_values,
                "selectedMissingValues": missing_values,
                "unselectedRowsNotDecoded": dict(sorted(unselected_rows.items())),
            },
            "outputs": {
                "observationArchiveSha256": _sha256(observation_archive),
                "trajectoryGenesSha256": role_contract.trajectory_genes_sha256,
                "trajectoryGeneSetSha256": role_contract.trajectory_gene_set_sha256,
                "actionSequenceSha256": action_sequence_sha256,
                "locatorSequenceSha256": locator_sequence_sha256,
            },
            "limitations": [
                "the module only normalizes protected truth and has no fitting or reward operation",
                ...
                "this preparation is not an admitted slp.corpus/v1.1 snapshot",
            ],
        }
        _write_json(publication_root / "preparation-audit.json", audit)
        result = {
            "audit": audit,
            "auditSha256": _sha256(publication_root / "preparation-audit.json"),
            ...
            "missingValues": missing_values,
            "unselectedRowsNotDecoded": sum(unselected_rows.values()),
        }
