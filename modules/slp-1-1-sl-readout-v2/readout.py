"""Frozen MuSL readout comparing static control with world augmentation."""
...
class StaticPairFeatures:
    """Symmetric raw-descriptor features computed only for selected pair rows."""
    def __init__(self, genes_path: Path, pairs_path: Path):
        with np.load(genes_path, allow_pickle=False) as genes:
            self.gene_ids = genes["gene_ids"].astype(str)
            self.features = np.asarray(genes["action_features"], np.float32)
        with np.load(pairs_path, allow_pickle=False) as pairs:
            self.pair_ids = pairs["pair_ids"].astype(str)
            self.indices = pairs["gene_indices"].astype(np.int64)
        if self.features.shape != (len(self.gene_ids), 642):
            raise ValueError("genes must contain 642 raw descriptors per stable ID")
        if self.indices.shape != (len(self.pair_ids), 2):
            raise ValueError("invalid canonical pair indices")
        if np.any(self.indices < 0) or np.any(self.indices >= len(self.gene_ids)):
            raise ValueError("pair gene index out of range")
        reconstructed = self.gene_ids[self.indices]
        if not np.array_equal(reconstructed, self.pair_ids):
            raise ValueError("pair IDs do not match indexed gene IDs")
        self.shape = (len(self.pair_ids), 1926)

    def __getitem__(self, rows):
        left = self.features[self.indices[rows, 0]]
        right = self.features[self.indices[rows, 1]]
...
def load_features(baseline_path: Path, world_path: Path, roster: Path,
                  genes_path: Path, pairs_path: Path):
    pair_ids = np.load(pairs_path, allow_pickle=False)["pair_ids"].astype(str)
    if sha256(pairs_path) != sha256(roster / "pairs.npz"):
        raise ValueError("--pairs is not the canonical roster pair file")
    if sha256(genes_path) != sha256(roster / "genes.npz"):
        raise ValueError("--genes is not the canonical roster gene file")
    baseline = BaselineFeatures(baseline_path, len(pair_ids))
    static = StaticPairFeatures(genes_path, pairs_path)
    ...
    return pair_ids, baseline, static, world
...
def fit(args) -> None:
    ...
    pair_ids, baseline, static, world = load_features(
        args.baseline, args.world, args.roster, args.genes, args.pairs)
    source_dir = Path(__file__).resolve().parent
    baseline_files = baseline.paths + ([args.baseline / "manifest.json"] if args.baseline.is_dir() else [])
    input_files = [*baseline_files, args.world, args.genes, args.pairs,
                   args.roster / "pairs.npz",
    ...
    for seed in SEEDS:
        ...
        for fold in FOLDS:
            ...
            xs = np.concatenate((xb, static[train_idx]), axis=1)
            xw = np.concatenate((xs, world[train_idx]), axis=1)
            mb, tb = _fit_lgb(xs[fit_mask], y[fit_mask], xs[val_mask], y[val_mask])
            ma, ta = _fit_lgb(xw[fit_mask], y[fit_mask], xw[val_mask], y[val_mask])
            pb = mb.predict_proba(xs[val_mask])[:, 1]
            ...
            mb = _refit_lgb(xs, y, tb)
            ma = _refit_lgb(xw, y, ta)
            prefix = f"seed{seed}-fold{fold}"
            mb.booster_.save_model(str(output / f"{prefix}-static-control.txt"))
            ma.booster_.save_model(str(output / f"{prefix}-world-augmented.txt"))
            test_b = baseline[test_idx]
            test_s = np.concatenate((test_b, static[test_idx]), axis=1)
            test_a = np.concatenate((test_s, world[test_idx]), axis=1)
            pred_b = mb.predict_proba(test_s)[:, 1].astype(np.float64)
            ...
            np.savez_compressed(output / f"{prefix}-predictions.npz",
                ...
                static_control=pred_b, world_augmented=pred_a,
            ...
            runs.append(dict(seed=seed, fold=fold, outer_train=int(train.sum()),
                ...
                static_control_trees=tb, world_augmented_trees=ta,
                world_augmented_blend_weight=weight,
                inner_logloss=float(min(losses))))
            print(json.dumps(dict(event="fit_fold", seed=seed, fold=fold,
                static_control_trees=tb, world_augmented_trees=ta, blend=weight,
...
def score(args) -> None:
    ...
    for seed in SEEDS:
        ...
        for fold in FOLDS:
            ...
            with np.load(pred_path, allow_pickle=False) as p:
                ...
                forecasts = {k: p[k].astype(np.float64) for k in
                             ("static_control", "world_augmented", "blend")}
    ...
    macro = []
    for arm in ("static_control", "world_augmented", "blend"):
...
def main() -> None:
    ...
    if args.phase == "fit":
        if any(x is None for x in (args.baseline, args.world, args.genes, args.pairs)):
            parser.error("fit requires --baseline, --world, --genes, and --pairs")
        fit(args)
    else:
        score(args)
