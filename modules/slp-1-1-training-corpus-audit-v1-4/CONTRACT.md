...
action taxa, and redundant corpus counts are recomputed. A snapshot may expose
the files directly or contain exactly one uncompressed `corpus-v1-2.tar` with
one `composite-corpus/` payload tree.

The output directory contains exactly one canonical `corpus-audit.json` using
`slp.corpus-audit/v1.4`. It contains only the pretrain corpus identity,
...
trainer remain historical contracts and cannot consume v1.4 evidence. New
