...
def _materialized_corpus_payload(
    ...
    if len(files) != 1:
        raise CorpusAuditError(
            "pretrain must contain a direct corpus or exactly one tar bundle"
        )
    relative = next(iter(files))
    if relative != "corpus-v1-2.tar":
        raise CorpusAuditError(
            "the sole pretrain bundle must be named corpus-v1-2.tar"
        )
...
def _load_corpus_root(
    ...
    return Corpus(
        dataset, identity, frozenset(interventions), frozenset(active_actions)
    )
