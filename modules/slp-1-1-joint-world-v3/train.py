...
@torch.no_grad()
def predict_parent_queries(model, data, parent_rows, observation_indices, decode_indices):
    """Generate only the query union consumed by this update; never read parent targets."""
    rows = np.asarray(parent_rows, np.int64)
    union = np.union1d(observation_indices, decode_indices)
    basal = data['basal'][rows]
    count = len(rows)
    device = basal.device
    modes = torch.full((count,), data['mode'], dtype=torch.long, device=device)
    assays = torch.full_like(modes, data['assay'])
    observed_mask = torch.ones((count, len(observation_indices)), dtype=torch.bool, device=device)
    scale = data['response_scale']
    initial = model.encode(basal[:, observation_indices]/scale, basal[:, observation_indices]/scale,
        data['query_features'][observation_indices], observed_mask, modes, assays,
        data['control_context_values'][rows][:, observation_indices],
        data['control_context_mask'][rows][:, observation_indices])
    changed = model.transition(initial, data['action_features'][rows], data['action_mask'][rows], modes, assays)
    correction = model.decode(changed, data['query_features'][union], assays) - model.decode(
        initial, data['query_features'][union], assays)
    return basal[:, union] + data['prior_per_action'][rows].sum(1)[:, union] + correction*scale
...
def main():
    ...
    metadata = {'schema': 'slp.joint-population-world/v3', 'config': asdict(config),
        ...
        'training': {'combination_fold': args.fold, 'steps_requested': args.steps,
            ...
            'rollout_ramp': args.rollout_ramp, 'rollout_gradient': 'stop-gradient generated parent',
            'objective': 'population endpoint residual MSE with scheduled predicted-parent observations plus masked state reconstruction',
    ...
    for step in range(1, args.steps+1):
        ...
        prior = data['prior_per_action'][rows].sum(1) - data['template_saturation'] * (
            action_mask.sum(1)-1)[:, None] * data['prior_template'][None, :]
        generated_positions, generated_parents = [], []
        rollout_probability = args.rollout_fraction * min(1., max(0.,
            (step - args.rollout_warmup) / args.rollout_ramp))
        if data['is_combinatorial']:
            # Learn both direct sets and one remaining action from measured or self-generated parents.
            for position, row in enumerate(rows):
                ...
                if parents[0] >= 0 and rng.random() < 0.67:
                    ...
                    if rng.random() < rollout_probability:
                        ...
                        generated_parents.append(parent_row)
        if generated_positions:
            generated = predict_parent_queries(model, data, generated_parents, obs_idx, out_idx)
            union = np.union1d(obs_idx, out_idx)
            positions = torch.as_tensor(generated_positions, device='cuda')
            query_indices = torch.as_tensor(union, device='cuda')
            observed[positions[:, None], query_indices[None, :]] = generated
        ...
        losses.append({'step': step, 'source': source, 'forecast': float(forecast_loss.detach()),
                       ...
                       'generated_parents': len(generated_positions), 'rollout_probability': rollout_probability,
