...
class JointWorldBundle:
    def __init__(self, modeldir, checkpoint=None, device='cpu'):
        ...
        if checkpoint is None:
            ...
            if manifest_path.is_file():
                checkpoint = json.loads(manifest_path.read_text(encoding='utf-8')).get('selectedCheckpoint')
            if checkpoint is None:
                completed = int(payload.get('training', {}).get('steps_completed', 0))
                if completed <= 0:
                    raise ValueError('specify a checkpoint for an unfinished training run')
    ...
    def _predict(self, context, action_features, action_mask, basal, observed, *, batch_size, query_chunk,
        ...
        prior = apply_prior_increment(flat, self.priors[context].intercept, mask,
            from_control, float(record.get('template_saturation', 0.)))
...
def main():
    p=argparse.ArgumentParser(); p.add_argument('--model',type=Path,required=True); p.add_argument('--checkpoint',help='defaults to the export selection or completed training step'); p.add_argument('--context',required=True); p.add_argument('--input',type=Path,required=True); p.add_argument('--output',type=Path,required=True); a=p.parse_args()
