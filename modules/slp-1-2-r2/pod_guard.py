...
Only the pod named by RUNPOD_POD_ID can be deleted. Completed artifacts are
verified in R2 before cleanup. The account-wide API key is never sent here.
"""
...
def main(args):
    ...
    init = dict(
        x.split(b"=", 1)
        for x in Path("/proc/1/environ").read_bytes().split(b"\0")
        if b"=" in x
    )
    ...
    identity = request(
        "query { pod(input: {podId: " + json.dumps(pod_id) + "}) { id } }"
    )
    ...
    print(
        json.dumps(
            {
                "event": "pod_guard_armed",
                "pod_id": pod_id,
                "terminate_at": args.deadline,
                "scoped_credential_verified": True,
            }
        ),
        flush=True,
    )
    ...
    while True:
        try:
            request(
                "mutation { podTerminate(input: {podId: " + json.dumps(pod_id) + "}) }"
            )
            print(
                json.dumps({"event": "self_terminate", "submitted": True}), flush=True
            )
            return
