...
Twenty-two focused local tests and three real-data CUDA readiness runs have
...
exercised.
The separate full optimizer-state R2 persistence check awaits payload
authorization; Costanzo admission and research launch remain on hold. The
preparation pod has been deleted; repeating the pending remote optimizer backup
check will require a fresh disposable run. Native RunPod execution must be
...
Neither controller provisions or launches research compute.
