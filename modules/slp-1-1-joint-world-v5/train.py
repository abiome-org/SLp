...
from response_model import fit, save, calibrate_template_saturation
...
def validate_combinations(data, source):
    """Validate raw simultaneous-pair and measured-single parent alignment."""
    combinations = np.asarray(data['combination_rows'])
    parents = np.asarray(data['combination_single_rows'])
    n = len(data['targets'])
    if combinations.ndim != 1 or parents.shape != (len(combinations), 2):
        raise ValueError(f'{source}: invalid combination row/parent shapes')
    if (np.any(combinations < 0) or np.any(combinations >= n) or
            np.any(parents < 0) or np.any(parents >= n)):
        raise ValueError(f'{source}: combination parent index out of range')
    mask = np.asarray(data['action_mask'], bool)
    features = np.asarray(data['action_features'])
    basal = np.asarray(data['basal'])
    single_set = set(np.asarray(data['single_rows']).tolist())
    for combination, pair_parents in zip(combinations, parents):
        if mask[combination].sum() != 2:
            raise ValueError(f'{source}: combination row must contain exactly two active actions')
        for slot, parent in enumerate(pair_parents):
            if int(parent) not in single_set or mask[parent].sum() != 1:
                raise ValueError(f'{source}: each combination parent must be a declared single action')
            parent_feature = features[parent, mask[parent]][0]
            if not np.array_equal(features[combination, slot], parent_feature):
                raise ValueError(f'{source}: pair action slot does not match its parent descriptor')
            if not np.array_equal(basal[combination], basal[parent]):
                raise ValueError(f'{source}: pair and parent basal states do not align')


def compose_action_prior(per_action, action_mask, saturation, template):
    """Compose calibrated action priors with exact zero for an empty action set."""
    counts = action_mask.sum(1)
    prior = (per_action * action_mask[..., None]).sum(1)
    prior = prior - saturation * (counts-1)[:, None] * template[None, :]
    return torch.where(counts[:, None] > 0, prior, torch.zeros_like(prior))
...
def prepare_context(path, output, fold, rng, observation_queries, response_rank):
    ...
    if is_combinatorial:
        ...
        data['parents'] = parent
        validate_combinations(data, source)
    else:
    ...
    data['prior_template'] = np.asarray(prior.intercept, np.float32)
    data['template_saturation'] = calibrate_template_saturation(
        data['prior_per_action'], data['targets'], data['basal'], data['action_mask'],
        data['fit_rows'], data['prior_template'])
    data['prior_shrinkage'] = float(prior.shrinkage)
...
def centered_direction_loss(prediction, target):
    """Match perturbation-specific direction after removing the batch mean effect."""
    ...
    eligible = torch.linalg.vector_norm(target, dim=1) > 1e-4
    cosine = torch.nn.functional.cosine_similarity(prediction, target, dim=1, eps=1e-4)
    return ((1-cosine)*eligible).sum() / eligible.sum().clamp_min(1)
...
def main():
    ...
    parser.add_argument('--state-slots', type=int, default=16)
    parser.add_argument('--rollout-fraction', type=float, default=0.5)
    parser.add_argument('--rollout-warmup', type=int, default=1000)
    parser.add_argument('--rollout-ramp', type=int, default=4000)
    ...
    if not 0 <= args.rollout_fraction <= 1 or args.rollout_warmup < 0 or args.rollout_ramp <= 0:
        raise ValueError('invalid rollout schedule')
    ...
    config = Config(feature_dim=len(mean), width=args.width, state_slots=args.state_slots, control_context=True,
    ...
    metadata = {'schema': 'slp.joint-population-world/v5', 'config': asdict(config),
        'observation_queries': args.observation_queries, 'seed': args.seed,
        'contexts': {name: {key: data[key] for key in ('mode', 'assay', 'response_scale', 'template_saturation', 'prior_shrinkage')}
        ...
        'training': {'combination_fold': args.fold, 'steps_requested': args.steps,
            ...
            'rollout_fraction': args.rollout_fraction, 'rollout_warmup': args.rollout_warmup,
            'rollout_ramp': args.rollout_ramp,
            'rollout_gradient': 'full gradient through molecular-prior anchors and both latent transports',
            'objective': 'population endpoint residual MSE decoded relative to encoded full-prior anchors, with scheduled two-step latent transport chains',
    ...
    for data in contexts.values():
        ...
        for key in ('targets', 'basal', 'action_features', 'query_features', 'prior_per_action','prior_template','control_context_values'):
    ...
    for step in range(1, args.steps+1):
        ...
        prior = compose_action_prior(data['prior_per_action'][rows], action_mask,
                                     data['template_saturation'], data['prior_template'])
        chain_positions, chain_orders = [], []
        rollout_probability = args.rollout_fraction * min(1., max(0.,
            (step - args.rollout_warmup) / args.rollout_ramp))
        if data['is_combinatorial']:
            # Parent-selected rows use either a measured endpoint continuation or
            # a two-action latent chain from basal. Direct pair rows are unchanged.
            for position, row in enumerate(rows):
                parents = data['parents'][row]
                if parents[0] >= 0 and rng.random() < 0.67:
                    parent_side = int(rng.integers(2))
                    if rng.random() < rollout_probability:
                        chain_positions.append(position)
                        chain_orders.append((parent_side, 1-parent_side))
                    else:
                        parent_row = int(parents[parent_side])
                        observed[position] = data['targets'][parent_row]
        ...
        final_prior_state = model.encode((observed[:, obs_idx]+prior[:, obs_idx])/response_scale,
            basal[:, obs_idx]/response_scale, data['query_features'][obs_idx], mask, mode, assay,
            data['control_context_values'][rows][:,obs_idx], data['control_context_mask'][rows][:,obs_idx])
        changed = model.transport_action(state, state, final_prior_state,
            actions, action_mask, mode, assay)
        if chain_positions:
            positions = torch.as_tensor(chain_positions, dtype=torch.long, device='cuda')
            orders = torch.as_tensor(chain_orders, dtype=torch.long, device='cuda')
            selected_rows = torch.as_tensor(rows[chain_positions], dtype=torch.long, device='cuda')
            first_side = orders[:, 0]
            prior_a = data['prior_per_action'][selected_rows, first_side]
            prior_a_state = model.encode(
                (basal[positions][:, obs_idx]+prior_a[:, obs_idx])/response_scale,
                basal[positions][:, obs_idx]/response_scale, data['query_features'][obs_idx],
                mask[positions], mode[positions], assay[positions],
                data['control_context_values'][selected_rows][:,obs_idx],
                data['control_context_mask'][selected_rows][:,obs_idx])
            first_mask = torch.zeros_like(action_mask[positions])
            first_mask.scatter_(1, first_side[:, None], True)
            second_side = orders[:, 1]
            second_mask = torch.zeros_like(first_mask)
            second_mask.scatter_(1, second_side[:, None], True)
            state_a = model.transport_action(state[positions], state[positions], prior_a_state,
                actions[positions], first_mask, mode[positions], assay[positions])
            chained = model.transport_action(state_a, prior_a_state,
                final_prior_state[positions], actions[positions], second_mask,
                mode[positions], assay[positions])
            changed = changed.index_copy(0, positions, chained)
        delta = model.decode(changed, data['query_features'][out_idx], assay) - model.decode(
            final_prior_state, data['query_features'][out_idx], assay)
        ...
        predicted_response = (observed[:, out_idx] + prior[:, out_idx] - basal[:, out_idx])/response_scale + delta
        target_response = (target[:, out_idx] - basal[:, out_idx])/response_scale
        direction_loss = centered_direction_loss(predicted_response, target_response)
        loss = (forecast_loss + args.reconstruction_weight*reconstruction_loss
                + penalty*delta.square().mean() + args.direction_weight*direction_loss)
        ...
        losses.append({'step': step, 'source': source, 'forecast': float(forecast_loss.detach()),
                       ...
                       'latent_chains': len(chain_positions), 'rollout_probability': rollout_probability,
