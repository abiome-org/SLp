...
class JointWorldBundle:
    ...
    def _predict(self,context,action_features,action_mask,basal,observed,*,batch_size,query_chunk,control_context_values,control_context_mask,sequential):
        ...
        for start in range(0,b,batch_size):
            ...
            def encode_anchor(values):
                return self.model.encode(torch.as_tensor(values[:,indices]/scale,dtype=torch.float32,device=self.device),torch.as_tensor(base[sl][:,indices]/scale,dtype=torch.float32,device=self.device),torch.as_tensor(nq[indices],device=self.device),torch.ones((n,len(indices)),dtype=torch.bool,device=self.device),modes,assays,torch.as_tensor(cv[sl][:,indices],device=self.device),torch.as_tensor(cm[sl][:,indices],device=self.device))
            state=encode_anchor(obs[sl])
            actions=torch.as_tensor(na[sl],device=self.device); action_mask_t=torch.as_tensor(np.ascontiguousarray(mask[sl]),device=self.device)
            if sequential:
                current_anchor=state
                prefix_mask=np.zeros((n,a),dtype=bool)
                for step in range(a):
                    prefix_mask[:,step]=mask[sl,step]
                    cumulative=apply_prior_increment(flat[sl],self.priors[context].intercept,prefix_mask,from_control[sl],float(record.get('template_saturation',0.)))
                    next_anchor=encode_anchor(obs[sl]+cumulative)
                    step_mask=torch.zeros_like(action_mask_t); step_mask[:,step]=action_mask_t[:,step]
                    state=self.model.transport_action(state,current_anchor,next_anchor,actions,step_mask,modes,assays)
                    current_anchor=next_anchor
                changed=state; final_anchor=current_anchor
            else:
                final_anchor=encode_anchor(obs[sl]+prior[sl])
                changed=self.model.transport_action(state,state,final_anchor,actions,action_mask_t,modes,assays)
            pieces=[]
            for qs in range(0,q,query_chunk):
                qe=min(qs+query_chunk,q); features=torch.as_tensor(nq[qs:qe],device=self.device)
                pieces.append((self.model.decode(changed,features,assays)-self.model.decode(final_anchor,features,assays)).cpu().numpy())
