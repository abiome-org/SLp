...
class SharedWorldModel(nn.Module):
    ...
    def __init__(self, config: Config):
        ...
        self.action_sum_body = nn.Sequential(nn.Linear(2*w, w), nn.GELU(), nn.Linear(w, w))
    ...
    def encode(self, observed_values, basal_values, query_features, observed_mask, mode_ids, assay_ids,
        ...
        if self.config.bind_observation_values:
            ...
            if self.config.control_context:
                # Native control profiles are commonly several log-expression units.
                # A fixed scale preserves ordering without saturating this binding.
                tokens = tokens + gene_tokens * torch.tanh(context[..., None] / 4.)
    ...
    def transition(self, state, action_features, action_mask, mode_ids, assay_ids):
        ...
        # Attention is exchangeable but normalized over actions, so it cannot by
        # itself distinguish one action from two identical active actions.  The
        # masked sum retains cardinality and is scaled only by the fixed model
        # width, never by the number of active actions.
        action_sum = (tokens * action_mask[..., None]).sum(1) / math.sqrt(self.config.width)
        state_norm = self.transition_norm(state + update)
        summed = action_sum[:, None, :].expand(-1, self.config.state_slots, -1)
        pooled_update = self.action_sum_body(torch.cat((state_norm, summed), -1))
        hidden = self.transition_body(state_norm + pooled_update)
        delta = self.transition_output(hidden)
        return torch.where(active[:,None,None], state + delta, state)

    def transition_chain(self, state, action_features, action_mask, order, mode_ids, assay_ids):
        """Apply action slots sequentially in an explicit per-row order."""
        if order.shape != action_mask.shape or order.dtype != torch.long:
            raise ValueError("order must be int64 [B,A]")
        if (order < 0).any() or (order >= action_mask.shape[1]).any():
            raise ValueError("order contains an invalid action slot")
        if not torch.equal(torch.sort(order, dim=1).values,
                           torch.arange(action_mask.shape[1], device=order.device)[None].expand_as(order)):
            raise ValueError("each order row must permute every action slot")
        changed = state
        for step in range(action_mask.shape[1]):
            selected = order[:, step]
            step_mask = torch.zeros_like(action_mask)
            step_mask.scatter_(1, selected[:, None],
                               action_mask.gather(1, selected[:, None]))
            changed = self.transition(changed, action_features, step_mask, mode_ids, assay_ids)
        return changed

    def transport_action(self, state, prior_state, next_prior_state,
                         action_features, action_mask, mode_ids, assay_ids):
        """Transport a learned residual state between known molecular-prior anchors."""
        if prior_state.shape != state.shape or next_prior_state.shape != state.shape:
            raise ValueError("state and prior anchors must have identical shapes")
        changed = self.transition(state, action_features, action_mask, mode_ids, assay_ids)
        transported = next_prior_state + (state - prior_state) + (changed - state)
        active = action_mask.any(1)
        return torch.where(active[:, None, None], transported, state)
