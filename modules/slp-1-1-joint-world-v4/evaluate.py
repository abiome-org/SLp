...
from response_model import apply_prior_increment
...
def prior_only(bundle,context,actions,mask,observed,include_template=True):
    ...
    return np.asarray(observed,np.float64)+apply_prior_increment(flat,bundle.priors[context].intercept,mask,
        np.full(len(actions),include_template,bool),bundle.settings['contexts'][context].get('template_saturation',0.))
...
def pair_forecasts(bundle, model_context, data, combination_indices):
    ...
    latent_ab=bundle.predict_latent_rollout(model_context,pair,pairmask,basal,**kw)
    latent_ba=bundle.predict_latent_rollout(model_context,pair[:,::-1].copy(),pairmask[:,::-1].copy(),basal,**kw)
    values={'directTwoActions':bundle.predict(model_context,pair,pairmask,basal,**kw),
        'latentAThenB':latent_ab,'latentBThenA':latent_ba,'latentAverage':.5*(latent_ab+latent_ba),
        ...
        'observedParentPrior':.5*(prior_only(bundle,model_context,bb,onemask,left,False)+prior_only(bundle,model_context,aa,onemask,right,False))}
...
def pair_report(truth,values,selection=None,group_ids=None):
    ...
    if group_ids is not None:
        report['latentVsPredictedAdditiveBootstrap']=grouped_mse_bootstrap(
            truth[take],values['latentAverage'][take],values['predictedAdditive'][take],np.asarray(group_ids)[take])
...
def main():
    ...
    latent_ab=bundle.predict_latent_rollout('norman',pair,pairmask,basal)
    latent_ba=bundle.predict_latent_rollout('norman',pair[:,::-1].copy(),pairmask[:,::-1].copy(),basal)
    values={'observedParentAThenB':observed_ab,'observedParentBThenA':observed_ba,'observedParentAverage':.5*(observed_ab+observed_ba),
            ...
            'observedParentPrior':.5*(prior_only(bundle,'norman',bb,onemask,left,False)+prior_only(bundle,'norman',aa,onemask,right,False))}
    ...
    reports['norman']['latentOrderRms']=float(np.sqrt(np.square(latent_ab[:,common]-latent_ba[:,common]).mean()))
    ...
    metrics['norman_latent_autonomous_mse']=norman_metrics['latentAverage']['mse']
    for context in ('gwps','hepg2'):
    ...
    if 'mcf10a_minimal_d6' in reports:
        ...
        metrics['mcf10a_minimal_d6_latent_pair_mse']=reports['mcf10a_minimal_d6']['pairs']['allPairs']['metrics']['latentAverage']['mse']
