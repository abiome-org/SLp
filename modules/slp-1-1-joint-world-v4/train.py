...
def main():
    ...
    metadata = {'schema': 'slp.joint-population-world/v4', 'config': asdict(config),
        ...
        'training': {'combination_fold': args.fold, 'steps_requested': args.steps,
            ...
            'rollout_gradient': 'full gradient through both latent transitions',
            'objective': 'population endpoint residual MSE with scheduled two-step latent action chains plus measured-parent and direct paths',
    ...
    for step in range(1, args.steps+1):
        ...
        if chain_positions:
            ...
            chained = model.transition_chain(state[positions], actions[positions],
                action_mask[positions], orders, mode[positions], assay[positions])
