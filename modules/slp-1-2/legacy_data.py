"""SLp-1.1 corpus readers retained for the version-pinned bootstrap corpus.

Adapted from slp-1-1-cell-world-v1/data.py at d668aae (MIT). New model code
does not load any SLp-1.1 weights or molecular simulation signatures.
"""
...
class Yeast(Source):
    ...
    def draw(self,rng,batch,enc,out):
        eligible=getattr(self,'eligible_rows',None)
        rows=rng.integers(len(self.target),size=batch) if eligible is None else rng.choice(eligible,size=batch)
        cols,ei,di=self.columns(rng,enc,out)
