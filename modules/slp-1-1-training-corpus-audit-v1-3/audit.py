"""Fail-closed v1.3 clean-training corpus and held-boundary audit.
...
AUDIT_SCHEMA = "slp.corpus-audit/v1.3"
...
def audit_training_corpus(
    ...
    *,
    ...
    if reward_enabled is not False:
        raise CorpusAuditError(
            "training corpus-audit v1.3 requires rewardEnabled=false; molecular "
    ...
    leaked = sorted((pretrain.trajectory_genes | pretrain.active_actions) & held_union)
    if leaked:
        raise CorpusAuditError(
            "held validation/final interventions occur in quantitative fitting "
            "trajectories; pretrain=" + ", ".join(leaked)
        )
