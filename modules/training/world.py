...
ROOT = next(
    (parent for parent in (Path(__file__).resolve().parent, *Path(__file__).resolve().parents) if (parent / "model").is_dir()),
    Path(__file__).resolve().parent,
)
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))
from model.v1.world import SLPredict
from model.v1.checkpoint import write_checkpoint_manifest
from modules.decoders.continuous import (
...
from modules.decoders.architectures import (
    DependencyActionAdapter,
    DiagonalActionCalibration,
    GraphHead,
    LowRankActionRotation,
    MultiActionComposition,
    ResidualEndpoint,
    SourceEndpoint,
    SymmetricHead,
    SymmetricPairFusion,
    SymmetricPairLatentFusion,
    SymmetricQGIHead,
    dependency_landscape_head,
    pair_transition_adapter,
)
from modules.evaluation.protocol import require_explicit_benchmark

SEED = 123


def save_world_checkpoint(model, output_dir):
    """Write a content-addressed model artifact without involving an evaluator."""
...
def pretraining_relation_data(pack):
    """Return a training-only relation pool from a feature pack.

    Older artifacts have no dedicated pool and remain runnable for historical
    reproduction. Newly prepared packs keep benchmark pair membership outside
    this pool.
    """
    if "pretrain_pairs" in pack.files:
        return pack["pretrain_pairs"], pack["pretrain_relations"]
    return pack["pairs"], pack["relations"]
...
def perturbseq_loss(model,decoder,genes,data,device,singles=None,residual_weight=0.,source_context=False,state_block=None,single_only=False):
    model.eval(); decoder.eval(); keep=np.flatnonzero((data["role"]==1)&((data["cardinality"]==1) if single_only else True)); total=pair_total=0.; seen=pairs_seen=0; state_dim,fitness_index=perturbseq_layout(data)
...
def pretrain_perturbseq(model,gene_state,data,device,epochs=10,rl_epochs=3,batch=512,residual_weight=0.,context_selection=False,learning_rate=3e-4,rl_learning_rate=2e-5,single_only=False):
    ...
    lookup={(int(s),int(a)):i for i,(s,(a,b)) in enumerate(zip(data["source"],data["pairs"])) if b<0}; singles=np.asarray([[lookup.get((int(s),int(a)),-1),lookup.get((int(s),int(b)),-1)] for s,(a,b) in zip(data["source"],data["pairs"])],"int64"); train=np.flatnonzero((data["role"]==0)&((data["cardinality"]==1) if single_only else True)); source=data["source"][train]; double=data["cardinality"][train]==2; weights=1/np.maximum(1,np.bincount(source)[source]); weights*=np.where(double,5.,1.); weights/=weights.sum(); opt=torch.optim.AdamW([*filter(lambda p:p.requires_grad,model.parameters()),*decoder.parameters()],learning_rate,weight_decay=1e-3); best=None; best_score=float("inf")
    for epoch in range(epochs):
        ...
        val,pair=perturbseq_loss(model,decoder,genes,data,device,singles,residual_weight,single_only=single_only); source_val,source_pair=perturbseq_loss(model,decoder,genes,data,device,singles,residual_weight,True,single_only=single_only); score=val+pair+(source_val+source_pair if context_selection else 0)
    ...
    model.load_state_dict(best[0]); decoder.load_state_dict(best[1]); opt=torch.optim.AdamW([*filter(lambda p:p.requires_grad,model.parameters()),*decoder.parameters()],rl_learning_rate,weight_decay=1e-3)
...
def perturbseq_block_scores(model,decoder,gene_state,data,device,single_only=False):
    ...
    for block,end in enumerate(data["state_blocks"]):
        val,pair=perturbseq_loss(model,decoder,genes,data,device,state_block=block,single_only=single_only); source_val,source_pair=perturbseq_loss(model,decoder,genes,data,device,source_context=True,state_block=block,single_only=single_only); result[f"state_block_{int(end)}_score"]=val+pair+source_val+source_pair
    return result
...
def fit_residual_endpoint(data_dir,model_path,decoder_path,out_dir,epochs=10,stochastic_epochs=3,d=384,latent=128,layers=6,perturb_name="perturbseq_world_v3_nested96.npz"):
    ...
    endpoint.load_state_dict(best); out=Path(out_dir); save_world_checkpoint(endpoint,out); metrics={"parameters":sum(p.numel() for p in endpoint.parameters()),"trainable_parameters":sum(p.numel() for p in endpoint.parameters() if p.requires_grad),"best_combined_score":best_score,"history":history}; (out/"basal_metrics.json").write_text(json.dumps(metrics,indent=2)); return metrics
...
def run(data_dir, out_dir, pretrain_epochs=12, rl_epochs=3, cvs=(), d=384, latent=128, layers=6, feature_name="features.npz", outcome_name="slkb_outcomes.npz", allow_benchmark=False):
    if cvs:
        require_explicit_benchmark(allow_benchmark, "Benchmark")
    torch.manual_seed(SEED); np.random.seed(SEED); device="cuda" if torch.cuda.is_available() else "cpu"
    pack=np.load(Path(data_dir)/feature_name); slkb_path=Path(data_dir)/outcome_name
    ...
    relation_pairs, relation_targets = pretraining_relation_data(pack)
    pretrain(model,state,relation_pairs,relation_targets,device,pretrain_epochs)
    if slkb is not None: train_outcomes(model,state,slkb,device,max(2,pretrain_epochs//2))
    reinforce(model,state,relation_pairs,relation_targets,device,rl_epochs)
    if slkb is not None: reinforce_outcomes(model,state,slkb,device,rl_epochs)
    rows=evaluate(model,state,pack,np.load(Path(data_dir)/"splits.npz"),cvs,device) if cvs else []
    Path(out_dir).mkdir(parents=True,exist_ok=True)
    save_world_checkpoint(model,out_dir)
...
def run_pretrain(data_dir,out_dir,epochs=12,d=384,latent=128,layers=6,feature_name="features.npz"):
    ...
    pack=np.load(Path(data_dir)/feature_name)
    state=pack["state"].astype("float32")
    model=SLPredict(d,latent,layers,28,2,state.shape[1]).to(device)
    print(json.dumps({"device":device,"parameters":model.count(),"stage":"pretrain"}),flush=True)
    relation_pairs, relation_targets = pretraining_relation_data(pack)
    pretrain(model,state,relation_pairs,relation_targets,device,epochs)
    save_world_checkpoint(model,out_dir)
    valid=torch.nonzero(
        (torch.as_tensor(relation_pairs[:,0].astype("int64"))*1000003+
         torch.as_tensor(relation_pairs[:,1].astype("int64")))%20==0
    ).squeeze(1)
    return {
        "val_relation":relation_loss(
            model,
            torch.as_tensor(state),
            torch.as_tensor(relation_pairs.astype("int64")),
            torch.as_tensor(relation_targets,dtype=torch.float32),
            valid,
            device,
        )
    }
...
def resume_outcomes(data_dir,model_path,out_dir,mode="cold_joint",epochs=6,rl_epochs=3,d=384,latent=128,layers=6,feature_name="features.npz",outcome_name="slkb_outcomes.npz",evaluate_feng=False):
    ...
    rows=evaluate(model,state,pack,np.load(Path(data_dir)/"splits.npz"),(3,),device) if evaluate_feng else []; Path(out_dir).mkdir(parents=True,exist_ok=True); save_world_checkpoint(model,out_dir); Path(out_dir,"metrics.json").write_text(json.dumps(rows,indent=2)); return rows


def resume_depmap_world(data_dir,model_path,out_dir,dependency_epochs=3,outcome_epochs=6,rl_epochs=3,d=384,latent=128,layers=6,evaluate_feng=False):
    torch.manual_seed(SEED); np.random.seed(SEED); device="cuda"; pack=np.load(Path(data_dir)/"features_spectral_safe.npz"); state=pack["state"].astype("float32"); dep=np.load(Path(data_dir)/"depmap_world.npz",allow_pickle=True); outcome=np.load(Path(data_dir)/"slkb_outcomes_intervention_depmap_world.npz",allow_pickle=True); sd=torch.load(model_path,map_location="cpu",weights_only=True); model=SLPredict(d,latent,layers,sd["cell.weight"].shape[0],sd["outcome.weight"].shape[0],state.shape[1],outcome["context_state"].shape[1]).to(device); model.load_state_dict(sd,strict=False); dep_val=pretrain_dependency(model,state,dep,device,dependency_epochs); train_outcomes(model,state,outcome,device,outcome_epochs,cold=True); reinforce_outcomes(model,state,outcome,device,rl_epochs,cold=True); rows=evaluate(model,state,pack,np.load(Path(data_dir)/"splits.npz"),(3,),device) if evaluate_feng else []; Path(out_dir).mkdir(parents=True,exist_ok=True); save_world_checkpoint(model,out_dir); Path(out_dir,"metrics.json").write_text(json.dumps(rows,indent=2)); Path(out_dir,"dependency_metrics.json").write_text(json.dumps({"validation_huber":dep_val},indent=2)); return rows


def resume_perturbseq(data_dir,model_path,out_dir,perturb_epochs=10,perturb_rl_epochs=3,outcome_epochs=6,d=384,latent=128,layers=6,perturb_name="perturbseq_world.npz",residual_weight=0.,context_selection=False,feature_name="features_spectral_safe.npz",evaluate_feng=False):
    torch.manual_seed(SEED); np.random.seed(SEED); device="cuda"; pack=np.load(Path(data_dir)/feature_name); state=pack["state"].astype("float32"); perturb=np.load(Path(data_dir)/perturb_name,allow_pickle=True); outcome=np.load(Path(data_dir)/"slkb_outcomes_intervention_external.npz",allow_pickle=True); sd=torch.load(model_path,map_location="cpu",weights_only=True); model=SLPredict(d,latent,layers,sd["cell.weight"].shape[0],sd["outcome.weight"].shape[0],state.shape[1]).to(device); model.load_state_dict(sd); decoder,perturb_val=pretrain_perturbseq(model,state,perturb,device,perturb_epochs,perturb_rl_epochs,residual_weight=residual_weight,context_selection=context_selection); train_outcomes(model,state,outcome,device,outcome_epochs,cold=True,head_only=True); rows=evaluate(model,state,pack,np.load(Path(data_dir)/"splits.npz"),(3,),device,perturb_decoder=decoder) if evaluate_feng else []; Path(out_dir).mkdir(parents=True,exist_ok=True); save_world_checkpoint(model,out_dir); torch.save(decoder.state_dict(),Path(out_dir)/"perturb_decoder.pt"); Path(out_dir,"metrics.json").write_text(json.dumps(rows,indent=2)); Path(out_dir,"perturbseq_metrics.json").write_text(json.dumps({"validation_score":perturb_val},indent=2)); return rows


def resume_basal_perturbseq(data_dir,model_path,out_dir,dependency_epochs=3,perturb_epochs=10,perturb_rl_epochs=3,outcome_epochs=6,d=384,latent=128,layers=6,feature_name="features_spectral_safe.npz",evaluate_cv3=False,perturb_name="perturbseq_world_v3.npz",fit_outcome=True,perturb_learning_rate=3e-4,perturb_rl_learning_rate=2e-5,single_only=False):
    torch.manual_seed(SEED); np.random.seed(SEED); device="cuda"; pack=np.load(Path(data_dir)/feature_name); state=pack["state"].astype("float32"); dep=np.load(Path(data_dir)/"basal_context.npz"); perturb=np.load(Path(data_dir)/perturb_name); sd=torch.load(model_path,map_location="cpu",weights_only=True); model=SLPredict(d,latent,layers,sd["cell.weight"].shape[0],sd["outcome.weight"].shape[0],state.shape[1],dep["cell_state"].shape[1]).to(device); model.load_state_dict(sd,strict=False); dependency_val=pretrain_dependency(model,state,dep,device,dependency_epochs); decoder,perturb_val=pretrain_perturbseq(model,state,perturb,device,perturb_epochs,perturb_rl_epochs,context_selection=True,learning_rate=perturb_learning_rate,rl_learning_rate=perturb_rl_learning_rate,single_only=single_only); block_scores=perturbseq_block_scores(model,decoder,state,perturb,device,single_only); pairs=torch.as_tensor(pack["pairs"].astype("int64")); valid=torch.nonzero((pairs[:,0]*1000003+pairs[:,1])%20==0).squeeze(1); relation_val=relation_loss(model,torch.as_tensor(state),pairs,torch.as_tensor(pack["relations"],dtype=torch.float32),valid,device)
    if fit_outcome: train_outcomes(model,state,np.load(Path(data_dir)/"slkb_outcomes_intervention_external.npz",allow_pickle=True),device,outcome_epochs,cold=True,head_only=True)
    rows=evaluate(model,state,pack,np.load(Path(data_dir)/"splits.npz"),(3,),device,perturb_decoder=decoder) if evaluate_cv3 else []; Path(out_dir).mkdir(parents=True,exist_ok=True); save_world_checkpoint(model,out_dir); torch.save(decoder.state_dict(),Path(out_dir)/"perturb_decoder.pt"); Path(out_dir,"metrics.json").write_text(json.dumps(rows,indent=2)); metrics={"parameters":model.count(),"single_only":bool(single_only),"perturbseq_training_rows":int(((perturb["role"]==0)&((perturb["cardinality"]==1) if single_only else True)).sum()),"perturbseq_validation_rows":int(((perturb["role"]==1)&((perturb["cardinality"]==1) if single_only else True)).sum()),"dependency_validation_huber":dependency_val,"perturbseq_validation_score":perturb_val,"relation_validation_huber":relation_val,**block_scores}; Path(out_dir,"basal_metrics.json").write_text(json.dumps(metrics,indent=2)); return metrics if not evaluate_cv3 else rows
