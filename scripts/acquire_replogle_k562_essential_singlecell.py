"""Resume-safe bounded acquisition of Replogle K562 essential raw single cells."""
...
OUTPUT = ROOT / "data/sources/replogle-2022-k562-essential-singlecell-v1"
URL = "https://ndownloader.figshare.com/files/35773219"
FILENAME = "K562_essential_raw_singlecell_01.h5ad"
BYTES = 10_661_879_995
MD5 = "4f1122ce1c7f13299a68df6459a266d3"
SOURCE_VERSION = "figshare-plus:20029387:v1:file:35773219"
MIB = 1024 * 1024
GIB = 1024 * MIB
RANGE_BYTES = 64 * MIB
CONNECTIONS = 4
...
class Acquisition:
    def __init__(self, output: Path, maximum_seconds: float) -> None:
        ...
        self.resuming = self.ledger_path.exists()
    ...
    def _new_ledger(self) -> dict:
        return {
            ...
            "status": "partial", "source_version": SOURCE_VERSION, "url": URL,
            "filename": FILENAME, "bytes": BYTES, "expected_md5": MD5,
            "range_bytes": RANGE_BYTES, "maximum_connections": CONNECTIONS,
            "maximum_seconds": self.maximum_seconds, "completed_ranges": {},
        }
    ...
    def _fetch_once(self, start: int, end: int) -> dict:
        ...
        return {"start": start, "end": end, "bytes": received, "sha256": digest.hexdigest(), "seconds": time.monotonic() - began, "attempts": 1}
    ...
    def ranges(self) -> list[tuple[int, int]]:
        ...
        for start in range(0, BYTES, RANGE_BYTES):
            end = min(start + RANGE_BYTES, BYTES) - 1
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=CONNECTIONS) as pool:
            ...
            for count, future in enumerate(concurrent.futures.as_completed(futures), 1):
                ...
                if count % 8 == 0 or count == len(futures):
                    ...
                    print(json.dumps({"event": "progress", "bytes": completed, "fraction": completed / BYTES, "elapsedSeconds": elapsed}), flush=True)
        return results
    ...
    def profile(self) -> dict:
        ...
        value = {"schema": "slp.range-throughput-profile/v1", "connections": CONNECTIONS, "rangeBytes": RANGE_BYTES, "bytes": transferred, "seconds": elapsed, "bytesPerSecond": throughput, "projectedRemainingDownloadPlusHashSeconds": projected, "maximumSeconds": self.maximum_seconds}
        atomic_json(self.profile_path, value)
        ...
        print(json.dumps({"event": "profile", **value}), flush=True)
        return value
    ...
    def run(self) -> dict:
        ...
        atomic_json(self.ledger_path, self.ledger)
        value = {"schema": "slp.immutable-source-download/v1", "status": "complete-verified", "sourceVersion": SOURCE_VERSION, "url": URL, "path": self.final.as_posix(), "bytes": size, "md5": md5, "sha256": sha, "maximumConnections": CONNECTIONS, "rangeBytes": RANGE_BYTES, "downloadedRangesThisRun": len(results), "networkSecondsAfterProfile": network_seconds, "totalSeconds": total, "profile": profile}
        atomic_json(self.complete_path, value)
        return value
