...
notes/data/orthology.fetch.tsv (filename<TAB>url) and data/raw/orthology_extra/SHA256SUMS.
...
from slpbench.fetch import BIOMART_URL, biomart_query
...
def main(force: bool = False) -> None:
    ...
    for ds, rel in BIOMART.items():
        ...
        rows.append((rel, f"{BIOMART_URL} (POST, slpbench.fetch.biomart_query('{ds}'))"))
    Path("notes/data").mkdir(parents=True, exist_ok=True)
    Path("notes/data/orthology.fetch.tsv").write_text("".join(f"{r}\t{u}\n" for r, u in rows))
