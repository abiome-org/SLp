...
feature *families* that exist in any species from the shared bundle (data/interim/bundle/<species>/):
  sequence   : paralog identity (Ensembl 116 via slpbench.homology for human/scer/spom/dmel, DIAMOND/Ensembl paralog
               edges from slpbench.families_extra for the other species; 0 = not paralogs), is_paralog,
...
def paralog_edges(sp: str) -> pd.DataFrame:
    import os
    cwd = os.getcwd()
    os.chdir(slb.ROOT)  # slpbench modules use repo-relative data paths
    try:
        from slpbench import homology
        p = homology.paralogs().filter(pl.col("species") == sp).select("a", "b", "identity").to_pandas()
        if p.empty:
            from slpbench import families_extra
