...
# median of the model's own scores -- exactly what `slpbench eval --allow-missing` does; the fill is done here because
...
# Usage: eval.sh <name>   env: SLB_BENCH (default data/bench/slb1.2), SLB_SPLIT (default dev; test is refused here)
# Results dir: results/models for slb1.2, results/models/<bench name>/ otherwise.
set -euo pipefail
ROOT=$(cd "$(dirname "$0")/../../.." && pwd); cd "$ROOT"
SPLIT=${SLB_SPLIT:-dev}
export SLB_BENCH=${SLB_BENCH:-data/bench/slb1.2}
BN=$(basename "$SLB_BENCH"); D=results/models; [ "$BN" = "slb1.2" ] || D=results/models/$BN
...
PY
uv run slpbench eval "$TMP" --split "$SPLIT" --out "$D/$1_${SPLIT}.json" > "$D/$1_${SPLIT}.txt" 2>&1 || { cat "$D/$1_${SPLIT}.txt"; rm -f "$TMP"; exit 1; }
rm -f "$TMP"
