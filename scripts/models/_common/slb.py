...
Every adapter reads the SLB benchmark from $SLB_BENCH (default data/bench/slb1.2), never touches
hidden/, trains only on train.parquet labels, and writes results/models/<name>_<split>.parquet
...
# results/models/ for SLB-1.2 (historical layout), results/models/<bench>/ for later versions (lead, 2026-09-24)
RESULTS = ROOT / "results/models" if BENCH.name == "slb1.2" else ROOT / "results/models" / BENCH.name
...
def evaluate(name: str, split: str = "dev") -> None:
    """Run slpbench eval on dev and save the report to results/models/<name>_dev.txt (no-op for other splits)."""
    if split != "dev":
        return None
    p = RESULTS / f"{name}_{split}.parquet"
    r = subprocess.run(["uv", "run", "slpbench", "eval", str(p), "--split", split, "--allow-missing",
