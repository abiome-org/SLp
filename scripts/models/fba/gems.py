...
see notes/models/fba.md for sources, versions and licences.
"""
...
def spne(medium: str = "default"):
    ...
    # canonical SLB IDs: shared resolver (slpbench.sources.bacteria_extra.spne_resolver, exported to TSV)
    res = {}
...
def _tsv_map(sp: str) -> dict[str, str]:
    p = ANNOT / f"{sp}_gem_map.tsv"  # written by export_maps.py (needs slpbench resolvers)
    with open(p) as fh:
