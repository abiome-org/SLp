...
PAYLOAD_SUFFIXES = {'.pt', '.pth', '.ckpt', '.safetensors', '.npz', '.npy',
                    '.pkl', '.joblib', '.parquet', '.h5ad', '.h5', '.bin', '.uint16', '.whl'}
...
def audit(root=ROOT):
    ...
    for name in tracked:
        ...
        if p.parts[0] in {'.omf', 'ontology', '.venv', '.cache'}:
    ...
    return {'tracked_files': len(tracked),
            'tracked_by_directory': dict(sorted(collections.Counter(Path(n).parts[0] if '/' in n else '(root)' for n in tracked).items())),
