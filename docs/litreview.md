...
SL-Predict connects these lines of work by treating the cell as a stochastic, context-conditioned dynamical system. A pre-intervention state and explicit gene or drug action are passed through a learned transition, molecular measurements are decoded from the evolved state, and a separate outcome model estimates viability. The headline empirical objective is benchmark-wide SOTA, especially on strict two-new-gene and intervention-isolated cold start. The decisive test is whether SL-Predict beats the strongest direct pair classifier when both systems receive matched data, capacity, feature access, and tuning budgets. A convincing result also requires context holdouts, fold-local feature construction, experimentally measured negatives, additive and mechanistic baselines, and proof that SL labels do not leak into the transition or viability training path.  
